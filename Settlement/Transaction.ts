...
export namespace Transaction {
	...
	export function toCsv(transactions: Transaction[]): string {
		...
		for (const value of transactions)
			result += `"${value.authorization}","${value.reference}","${value.type}","${value.card}","${value.scheme}","${value.area}","${value.created}","${value.gross}","${value.fee}","${value.net}","${value.reserve?.amount}","${value.reserve?.amount}"\r\n`
		return result
	}
}
